...
def mixture_inv_cdf(y, prior_logits, means, log_scales,
    ...
    lb, _ = (means - 200 * max_scales).min(dim=1)
    ub, _ = (means + 200 * max_scales).max(dim=1)
    diff = float('inf')
...
def reverse_sigmoid(x):
    return -torch.log(torch.reciprocal(x) - 1.0)
...
class Sigmoid(torch.nn.Module):
    ...
    def inverse(self, x):
        y = torch.sigmoid(x)
        logd = - torch.nn.functional.softplus(x) - torch.nn.functional.softplus(-x)
        return y, logd

    def forward(self, y):
        x = -safe_log(torch.reciprocal(y) - 1.0)
